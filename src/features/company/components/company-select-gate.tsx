...
/**
 * Post-login company gate. When the user belongs to more than one company and
 * hasn't picked one (`requires_selection`), this blocks the app with a
 * non-dismissable modal until a company is chosen. Single-company users (and
 * anyone who's already selected) never see it — `requires_selection` is false.
 *
 * Selection is two-step: pick a company to highlight it, then Confirm. A Sign
    ...
    <Dialog open onOpenChange={() => {}}>
      <DialogContent showClose={false} className="max-w-md">
        ...
        {selectCompany.isError && (
          <p className="mt-3 text-sm text-destructive">{selectCompany.error.message}</p>
        )}
