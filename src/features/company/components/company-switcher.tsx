import { Building2, Check, ChevronDown, Loader2 } from 'lucide-react'
import {
  DropdownMenu,
  DropdownLabel,
  DropdownSeparator,
} from '@/components/ui/dropdown-menu'
import { Tooltip, TooltipContent, TooltipTrigger } from '@/components/ui/tooltip'
import { cn } from '@/lib/utils'
import { useMyCompanies } from '../api/use-my-companies'
import { useSelectCompany } from '../api/use-select-company'

/** Shared chip styling for the topbar trigger. */
const chip =
  'flex items-center gap-2 rounded-full border bg-background px-3 py-1.5 text-sm font-medium text-foreground'

/**
 * Active-company selector for the topbar (sits before the profile menu). Lists
 * the tenants the user belongs to and switches the active one via
 * `POST /user/auth/select-company`. Always renders a visible affordance while
 * authenticated — a loading chip, an error chip, or the company dropdown.
 */
export function CompanySwitcher() {
  const { companies, selectedCompanyId, isLoading, isError, error } = useMyCompanies()
  const selectCompany = useSelectCompany()

  if (isLoading) {
    return (
      <div className={cn(chip, 'text-muted-foreground')}>
        <Loader2 className="size-4 animate-spin" />
        <span className="hidden xl:block">Loading…</span>
      </div>
    )
  }

  if (isError) {
    return (
      <Tooltip>
        <TooltipTrigger asChild>
          <div className={cn(chip, 'border-destructive/40 text-destructive')}>
            <Building2 className="size-4" />
            <span className="hidden xl:block">Companies unavailable</span>
          </div>
        </TooltipTrigger>
        <TooltipContent side="bottom">
          {error?.message ?? 'Companies unavailable'}
        </TooltipContent>
      </Tooltip>
    )
  }

  if (companies.length === 0) return null

  const activeId = selectedCompanyId
  const active = companies.find((c) => c.id === activeId)
  const label = active?.name ?? 'Select company'

  return (
    <DropdownMenu
      className="min-w-56 p-1"
      trigger={
        <Tooltip>
          <TooltipTrigger asChild>
            <button
              type="button"
              className={cn(chip, 'cursor-pointer transition-colors hover:bg-accent')}
            >
              <Building2 className="size-4 text-muted-foreground" />
              <span className="hidden max-w-40 truncate xl:block">{label}</span>
              <ChevronDown className="size-4 text-muted-foreground" />
            </button>
          </TooltipTrigger>
          <TooltipContent side="bottom">Switch company</TooltipContent>
        </Tooltip>
      }
    >
      <DropdownLabel>Companies</DropdownLabel>
      <DropdownSeparator />
      {companies.map((company) => {
        const isActive = company.id === activeId
        return (
          <button
            key={company.id}
            type="button"
            disabled={selectCompany.isPending}
            onClick={() => {
              if (!isActive) selectCompany.mutate(company.id)
            }}
            className={cn(
              'flex w-full cursor-pointer items-center gap-2 rounded-md px-2.5 py-2 text-left text-sm hover:bg-accent hover:text-accent-foreground disabled:cursor-not-allowed disabled:opacity-60',
              isActive && 'font-semibold',
            )}
          >
            <Building2 className="size-4 shrink-0 text-muted-foreground" />
            <span className="flex-1 truncate">{company.name}</span>
            {isActive && <Check className="size-4 shrink-0 text-primary" />}
          </button>
        )
      })}
    </DropdownMenu>
  )
}
