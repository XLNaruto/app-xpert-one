...
/**
 * Create/edit a branch record. One screen for both: pass `branchId` to edit an
 * existing record (hydrates the form + updates on submit), or omit it to create
 * a new one. Both tabs sit inside a single form, so one submit saves the branch
 * detail and every applicable act together.
 */
export function BranchManagePage({ branchId }: { branchId?: number }) {
  const {
    register,
    control,
    errors,
