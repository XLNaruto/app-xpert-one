...
interface EmploymentExchangeOfficeAddressFormFieldsProps {
  register: UseFormRegister<EmploymentExchangeOfficeAddressFormValues>
  control: Control<EmploymentExchangeOfficeAddressFormValues>
  errors: FieldErrors<EmploymentExchangeOfficeAddressFormValues>
...
/**
 * The employment exchange office address field grid — who the office is, how to reach it and
 * where it sits. District cascades off the selected state.
 */
export function EmploymentExchangeOfficeAddressFormFields({
  register,
  control,
  errors,
  stateOptions,
  ...
  hasState,
  changeState,
}: EmploymentExchangeOfficeAddressFormFieldsProps) {
  return (
    <div className="grid grid-cols-1 gap-x-6 gap-y-5 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
      <FormSection
        icon={Building2}
        title="Office Information"
        description="How this employment exchange office is identified"
        className="mt-0"
      />
