...
import { factoryOfficeAddressSchema, type FactoryOfficeAddressFormValues } from '../schemas'
import { EMPTY_FACTORY_OFFICE_ADDRESS_FORM } from '../constants'
import { useFactoryOfficeAddress } from '../api/use-factory-office-address'
import {
  useCreateFactoryOfficeAddress,
  useUpdateFactoryOfficeAddress,
} from '../api/use-factory-office-address-mutations'
import { factoryOfficeAddressToFormValues } from '../lib/factory-office-address-mappers'

/**
 * Owns the factory office address form for both create and edit. In edit mode
 ...
 * fields.
 */
export function useFactoryOfficeAddressForm(id?: number) {
  const isEdit = id !== undefined
  const navigate = useNavigate()

  const detail = useFactoryOfficeAddress(id ?? Number.NaN)
  const createAddress = useCreateFactoryOfficeAddress()
  const updateAddress = useUpdateFactoryOfficeAddress(id ?? Number.NaN)
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<FactoryOfficeAddressFormValues>({
    resolver: zodResolver(factoryOfficeAddressSchema),
    defaultValues: EMPTY_FACTORY_OFFICE_ADDRESS_FORM,
  })

  // Seed the form once the record loads (edit mode only).
  useEffect(() => {
    if (detail.data) reset(factoryOfficeAddressToFormValues(detail.data))
  }, [detail.data, reset])
  ...
  const goToList = () => navigate({ to: '/master/factory-office-address' })
  ...
  const onSubmit = handleSubmit((values) => {
    ...
    mutation.mutate(values, {
      onSuccess: () => {
        toast.success(
          isEdit ? 'factory office address updated' : 'factory office address created',
        )
        goToList()
      },
      onError: (err) =>
        toast.error(
          err instanceof Error
            ? err.message
            : `Failed to ${isEdit ? 'update' : 'create'} factory office address`,
        ),
    })
  })
